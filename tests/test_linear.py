...
if __name__ == "__main__":
    ...
    try:
        ...
        if "quota" in error_str or "rate limit" in error_str:
            ...
            print("=" * 60)
            print("⚠️  IMPORTANT: Rate limits ≠ Quota limits!")
            print("")
            print("Rate Limits (requests per minute):")
            print("  - Free tier: ~3 requests/minute (even with credits!)")
            print("  - Paid plans: Much higher limits (e.g., 500/min)")
            print("  - Solution: Wait 1-2 minutes and try again, or upgrade plan")
            print("")
            print("Quota Limits (spending limit):")
            print("  - Check billing: https://platform.openai.com/account/billing")
            print("  - Add payment method or credits")
            print("")
            print("Other solutions:")
            print("  - Reduce max_steps to make fewer API calls")
            print("  - The system is already using gpt-4o-mini (cheapest model)")
            print("  - Test with hardcoded steps: python tests/test.py")
            print("=" * 60)
